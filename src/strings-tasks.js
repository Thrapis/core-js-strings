...
function removeFirstOccurrences(str, value) {
  ...
  const len = firstOccur >= 0 && value.length;
  return (
    str.substring(0, firstOccur) + str.substring(firstOccur + len, str.length)
  );
}
...
function sumOfCodes(str) {
  let sum = 0;
  for (let i = 0; i < (str && str.length); i += 1) {
...
function encodeToRot13(str) {
  return str
    ...
    .map((c) => {
      ...
      let number = c.charCodeAt(0) - 65;
      if (number > 25) {
        isLowercase = true;
        number -= 32;
      }
      return String.fromCharCode(
        ((number + 13) % 26) + 65 + (isLowercase ? 32 : 0)
      );
    })
    .join('');
}
...
function getCardId(value) {
  ...
  for (let i = 0; i < rankReplacements[0].length; i += 1)
    rank = rank.replace(rankReplacements[0][i], rankReplacements[1][i]);
  return parseInt(rank, 10) + suit * 13 - 1;
}
